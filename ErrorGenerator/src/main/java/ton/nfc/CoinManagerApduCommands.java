...
import static ton.nfc.ResponsesConstants.ERROR_MSG_LABEL_BYTES_SIZE_INCORRECT;
import static ton.nfc.ResponsesConstants.ERROR_MSG_PIN_BYTES_SIZE_INCORRECT;
import static ton.nfc.TonWalletConstants.PIN_SIZE;
...
public class CoinManagerApduCommands {
    ...
    public static CAPDU getChangePinAPDU(byte[] oldPinBytes, byte[] newPinBytes) {
        ...
        byte[] data = BYTE_ARRAY_HELPER.bConcat(BYTE_ARRAY_HELPER.bytes(CHANGE_PIN_DATA), new byte[]{
                PIN_SIZE}, oldPinBytes, new byte[]{PIN_SIZE}, newPinBytes);
