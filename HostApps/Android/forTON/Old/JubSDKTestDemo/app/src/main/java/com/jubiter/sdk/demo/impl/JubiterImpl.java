...
//    }
    ...
    public void verifyPin(final JubCallback<String, Void> callback) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                ...
                mUIHandler.post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        String apduShowPinTable = "B0A10000"; // "B0 A1 00 00"
                        final String respShowPinTable = NativeApi.nativeSendApdu(mDeviceHandle, apduShowPinTable);
                        if (!respShowPinTable.equals("9000")) {
                            final int finalRet = Integer.parseInt(respShowPinTable);
                            mUIHandler.post(new Runnable() {
                                @Override
                                public void run() {
                                    callback.onFailed(finalRet);
                        ...
                        final VerifyPinDialog dialog = new VerifyPinDialog(mContext, new VerifyPinDialog.callback() {
                            ...
                            public void onClickListener(String pin) {
                                ...
                                final String apduVerifyPin = "B0A2000004" + ByteArrayHelper.pinToHex(pin);
                                final String respVerifyPin = NativeApi.nativeSendApdu(mDeviceHandle, apduVerifyPin);

                                if (!respVerifyPin.equals("9000")) {
                                    mUIHandler.post(new Runnable() {
                                        @Override
                                        public void run() {
                                            callback.onFailed(Integer.parseInt(respVerifyPin));
                                        }
                                    });
                                    return;
                                }
                                    ...
                                    mUIHandler.post(new Runnable() {
                                        @Override
                                        public void run() {
                                            callback.onSuccess(respVerifyPin, null);
                                        }
                                    });
                                    return;
                                }
