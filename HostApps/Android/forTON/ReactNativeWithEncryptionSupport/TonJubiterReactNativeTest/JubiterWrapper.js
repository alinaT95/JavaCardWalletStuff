...
export default class JubiterWrapper extends Component {
	...
	initializeCardKeyForDHProtocol() {
		NativeModules.JubiterModule.initializeCardKeyForDHProtocol((err , res) => {
			printResults(err, "initializeCardKeyForDHProtocol result: " + res);
		});
	}
	
	getCardPublicKeyForDHProtocol() {
		NativeModules.JubiterModule.getCardPublicKeyForDHProtocol((err , res) => {
			printResults(err, "getCardPublicKeyForDHProtocol result: " + res);
		});
	}
	
	getCommonSecretForDHProtocol() {
		NativeModules.JubiterModule.getCommonSecretForDHProtocol((err , res) => {
			printResults(err, "getCommonSecretForDHProtocolresult: " + res);
		});
	}
	
	encrypt(message, nonce, theirPublicKey) {
		NativeModules.JubiterModule.encrypt(
			message, nonce, theirPublicKey,
			(err , res) => {
				printResults(err, "encrypt result: " + res);
			}
		);
	}
}