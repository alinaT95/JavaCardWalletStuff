...
import com.tonjubiterreactnativetest.api.TonJubiterApi;
...
public class JubiterModule extends ReactContextBaseJavaModule {
    ...
    public void scan(){
        if (tonJubiterApi == null)
            tonJubiterApi = new TonJubiterApi(getCurrentActivity());
...
//    }
    ...
    @ReactMethod
    public void initializeCardKeyForDHProtocol(Callback cb){
        if (!isDeviceConnected())
            cb.invoke(DEVICE_IS_NOT_CONNECTED, null);
        else {
            String result = tonJubiterApi.initializeCardKeyForDHProtocol();
            cb.invoke(null, result);
        }
    }

    @ReactMethod
    public void getCardPublicKeyForDHProtocol(Callback cb){
        if (!isDeviceConnected())
            cb.invoke(DEVICE_IS_NOT_CONNECTED, null);
        else {
            String result = tonJubiterApi.getCardPublicKeyForDHProtocol();
            cb.invoke(null, result);
        }
    }

    @ReactMethod
    public void getCommonSecretForDHProtocol(Callback cb){
        if (!isDeviceConnected())
            cb.invoke(DEVICE_IS_NOT_CONNECTED, null);
        else {
            String result = tonJubiterApi.getCommonSecretForDHProtocol();
            cb.invoke(null, result);
        }
    }

    @ReactMethod
    public void encrypt(String message, String nonce, String theirPublicKey, Callback cb){
        if (!isDeviceConnected())
            cb.invoke(DEVICE_IS_NOT_CONNECTED, null);
        else {
            String result = tonJubiterApi.encrypt(message, nonce, theirPublicKey);
            cb.invoke(null, result);
        }
    }
