...
import static com.tonnfccardreactnativetest.api.utils.StringHelper.isHexString;
...
public class ByteArrayHelper {
    ...
    public static String hex(byte[] bytes) {
        if(bytes==null || bytes.length == 0) return  "";//throw new IllegalArgumentException("Source array is null");
    ...
    public static byte[] bytes(String s) {
        ...
        if(!isHexString(s)) throw new IllegalArgumentException("Source string is not correct hex: '"+s+"'");
        try {
            int len = s.length();
