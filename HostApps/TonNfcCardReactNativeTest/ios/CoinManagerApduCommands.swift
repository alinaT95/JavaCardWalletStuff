...
class CoinManagerApduCommands {
  static let LABEL_LENGTH = 32
  static let POSITIVE_ROOT_KEY_STATUS = "5a"
  ...
  static func getGenSeedApduPromise(pin : [UInt8]) -> Promise<NFCISO7816APDU> {
    Promise<NFCISO7816APDU>{ promise in promise.fulfill(try getGenSeedApdu(pin : pin))}
  }
  
  static func getChangePinApdu(oldPin : [UInt8], newPin :[UInt8]) throws -> NFCISO7816APDU {
    if oldPin.count !=  TonWalletAppletConstants.PIN_SIZE || newPin.count != TonWalletAppletConstants.PIN_SIZE {
      throw "Bad length of PIN (must be \(TonWalletAppletConstants.PIN_SIZE) bytes)."
    }
  ...
  static func getChangePinApduPromise(oldPin : [UInt8], newPin :[UInt8]) -> Promise<NFCISO7816APDU> {
    Promise<NFCISO7816APDU>{ promise in promise.fulfill(try getChangePinApdu(oldPin : oldPin, newPin : newPin))}
  }
  
  static func getSetDeviceLabelApdu(label : [UInt8]) throws -> NFCISO7816APDU {
    if label.count != LABEL_LENGTH  {
      throw "Bad length of device label (must be 32 bytes)."
    }
  ...
  static func getSetDeviceLabelApduPromise(label : [UInt8]) -> Promise<NFCISO7816APDU> {
    Promise<NFCISO7816APDU>{ promise in promise.fulfill(try getSetDeviceLabelApdu(label : label))}
  }
