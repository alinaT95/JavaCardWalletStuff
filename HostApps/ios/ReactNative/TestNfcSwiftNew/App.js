    ...
    return (
        <SafeAreaView style={styles.container}>
          <ScrollView
            ...
            <View>
              <Dialog.Container visible={this.state.isSetKeyForHmacDialogVisible}>
                ...
                <Dialog.Input label="Authentication password" defaultValue={this.state.authenticationPassword} style={{ height: 100 }} multiline={true} numberOfLines={4} onChangeText={(authenticationPassword) => this.setState({ authenticationPassword })}></Dialog.Input>
                <Dialog.Input label="Common secret" style={{ height: 70 }} defaultValue={this.state.commonSecret} multiline={true} numberOfLines={4} onChangeText={(commonSecret) => this.setState({ commonSecret })}></Dialog.Input>
                <Dialog.Button label="Close" onPress={() => this.showSetKeyForHmacDialog(false)} />
                <Dialog.Button label="Submit" onPress={() => {
                  nfcWrapper.setKeyForHmac(this.state.authenticationPassword, this.state.commonSecret)
            ...
            <View>
              <Dialog.Container visible={this.state.isChangePinDialogVisible}>
                ...
                <Dialog.Input label="Current pin" style={{ height: 40 }} defaultValue={this.state.currentPin} onChangeText={(currentPin) => this.setState({ currentPin })}></Dialog.Input>
                <Dialog.Input label="New pin" style={{ height: 40 }} defaultValue={this.state.newPin} onChangeText={(newPin) => this.setState({ newPin })}></Dialog.Input>
                <Dialog.Button label="Close" onPress={() => { this.showChangePinDialog(false) }} />
                <Dialog.Button label="Submit" onPress={() => {
            ...
            <View>
              <Button onPress={() => { this.showChangePinDialog(true) }} title="changePin" />
            </View>
            <Separator />
            <View>
            ...
            <View>
              <Dialog.Container visible={this.state.isTurnOnWalletDialogVisible}>
                ...
                <Dialog.Input label="Authentication password" defaultValue={this.state.authenticationPassword} style={{ height: 100 }} multiline={true} numberOfLines={4} onChangeText={(authenticationPassword) => this.setState({ authenticationPassword })}></Dialog.Input>
                <Dialog.Input label="Common secret" style={{ height: 70 }} defaultValue={this.state.commonSecret} multiline={true} numberOfLines={4} onChangeText={(commonSecret) => this.setState({ commonSecret })}></Dialog.Input>
                <Dialog.Input label="Initial vector" style={{ height: 40 }} defaultValue={this.state.initialVector} onChangeText={(initialVector) => this.setState({ initialVector })}></Dialog.Input>
                <Dialog.Button label="Close" onPress={() => this.showTurnOnWalletDialog(false)} />
                <Dialog.Button label="Submit" onPress={() => {
                  nfcWrapper.turnOnWallet(this.state.newPin, this.state.authenticationPassword, this.state.commonSecret, this.state.initialVector)
            ...
            <View>
              <Button onPress={() => {nfcWrapper.verifyHashOfEncryptedPassword(this.state.authenticationPassword, this.state.initialVector)}} title="verifyHashOfEncryptedPassword" />
            </View>
            <Separator />

            <View>
              <Button onPress={() => {nfcWrapper.verifyHashOfCommonSecret(this.state.commonSecret)}} title="verifyHashOfCommonSecret" />
            </View>
            <Separator />
            ...
            <View>
              <Button onPress={() => {nfcWrapper.getTonAppletState()}} title="getTonAppletState" />
            </View>
            <Separator />
            <View>
              <Button onPress={() => {nfcWrapper.getPublicKeyForDefaultPath()}} title="Get public key for default HD path" />
            </View>
            <Separator />
            {/*<View>*/}
            ...
            <View style={styles.container}>
              <Dialog.Container visible={this.state.isSignForDefHdPathDialogVisible}>
                ...
                <Dialog.Input label="Data to sign" style={{ height: 70 }} multiline={true} numberOfLines={4} defaultValue={this.state.data} onChangeText={(data) => this.setState({ data })}></Dialog.Input>
                <Dialog.Input label="Pin" style={{ height: 40 }} defaultValue={this.state.currentPin} onChangeText={(currentPin) => this.setState({ currentPin })}></Dialog.Input>
                <Dialog.Button label="Close" onPress={() => { this.showSignForDefaultHdPathDialog(false) }} />
                <Dialog.Button label="Submit" onPress={() => { nfcWrapper.signForDefaultHdPath(this.state.data, this.state.currentPin); this.showSignDialog(false); }} />
              </Dialog.Container>
            </View>
            <View>
            ...
            <View>
              <Dialog.Container visible={this.state.isSignDialogVisible}>
                ...
                <Dialog.Input label="Data to sign" style={{ height: 70 }} multiline={true} numberOfLines={4} defaultValue={this.state.data} onChangeText={(data) => this.setState({ data })}></Dialog.Input>
                <Dialog.Input label="Hd index" style={{ height: 40 }} defaultValue={this.state.hdIndex} onChangeText={(hdIndex) => this.setState({ hdIndex })}></Dialog.Input>
                <Dialog.Input label="Pin" style={{ height: 40 }} defaultValue={this.state.currentPin} onChangeText={(currentPin) => this.setState({ currentPin })}></Dialog.Input>
                <Dialog.Button label="Close" onPress={() => { this.showSignDialog(false) }} />
                <Dialog.Button label="Submit" onPress={() => {
          ...
          <ScrollView
            ...
            <View>
              <Button onPress={() => {
                NativeModules.CardKeyChainNfcModule.getKeyChainDataAboutAllKeys().then((result) => {
            ...
            <View>
              <Dialog.Container visible={this.state.isAddKeyIntoKeyChainDialogVisible}>
                ...
                <Dialog.Input label="Enter new key:" defaultValue={this.state.newKey} style={{ height: 100 }} multiline={true} numberOfLines={4} onChangeText={(newKey) => this.setState({ newKey })}></Dialog.Input>
                <Dialog.Button label="Close" onPress={() => this.showAddKeyIntoKeyChainDialog(false)} />
                <Dialog.Button label="Submit" onPress={() => {
                  NativeModules.CardKeyChainNfcModule.addKeyIntoKeyChain(this.state.newKey)
                      .then((res) => {
                        Alert.alert(
                            'Response from card',
            ...
            <View>
              <Dialog.Container visible={this.state.isChangeKeyFromKeyChainDialogVisible}>
                ...
                <Dialog.Input label="Old key index" style={{ height: 40 }} defaultValue={this.state.oldKeyIndex} onChangeText={(oldKeyIndex) => this.setState({ oldKeyIndex })}></Dialog.Input>
                <Dialog.Input label="New key" style={{ height: 100 }} defaultValue={this.state.newKeyForChangeKey} onChangeText={(newKeyForChangeKey) => this.setState({ newKeyForChangeKey })}></Dialog.Input>
                <Dialog.Button label="Close" onPress={() => this.showChangeKeyFromKeyChainDialog(false)} />
                <Dialog.Button label="Submit" onPress={() => {
                  NativeModules.CardKeyChainNfcModule.changeKeyInKeyChain(this.state.newKeyForChangeKey, this.state.keyData[this.state.oldKeyIndex].hmac)
                      .then((res) => {
                        Alert.alert(
                            'Response from card',
