...
class CardKeyChainNfcApi: TonNfcApi {
  ...
  func getKeyChainDataAboutAllKeys() {
    ...
    apduRunner.setCardOperation(cardOperation: { () in
      ...
      .then{(numOfKeys : Data) -> Promise<Data> in
        let numOfKeys = HexHelper.makeShort(src: numOfKeys.bytes, srcOff: 0)
        ...
        for ind in 0..<numOfKeys {
          ...
          .then { (hmacAndLen : Data) -> Promise<Data> in
            ...
            keyHmacsAndLens.append(["hmac" : Data(_ : hmacAndLen.bytes[range : 0..<TonWalletAppletConstants.HMAC_SHA_SIG_SIZE]).hexEncodedString(), "length" :
              String(HexHelper.makeShort(src: hmacAndLen.bytes, srcOff: TonWalletAppletConstants.HMAC_SHA_SIG_SIZE))
            ])
  ...
  func getKeyChainInfo() {
    ...
    apduRunner.setCardOperation(cardOperation: { () in
      ...
      .then{(numOfKeys : Data) -> Promise<Data> in
        keyChainInfo["numberOfKeys"] = String(HexHelper.makeShort(src: numOfKeys.bytes, srcOff: 0))
      ...
      .then{(occupiedSize : Data) -> Promise<Data> in
        keyChainInfo["occupiedSize"] = String(HexHelper.makeShort(src: occupiedSize.bytes, srcOff: 0))
        return self.getSaultPromise()
      }
      .then{(sault : Data) -> Promise<Data> in
      ...
      .then{(freeSize : Data) -> Promise<Data> in
        keyChainInfo["freeSize"] = String(HexHelper.makeShort(src: freeSize.bytes, srcOff: 0))
        return self.getSaultPromise()
      }
      .then{(response : Data) -> Promise<String> in
        Promise<String> { promise in
  ...
  func addKeyIntoKeyChain(newKey: String){
    ...
    let newKeyBytes = HexHelper.hexStrToUInt8Array(hexStr: newKey)
  ...
  func changeKeyInKeyChain(newKey : String, oldKeyHMac : String){
    ...
    let newKeyBytes = HexHelper.hexStrToUInt8Array(hexStr: newKey)
    let oldKeyHMacBytes = HexHelper.hexStrToUInt8Array(hexStr: oldKeyHMac)
    ...
    apduRunner.setCardOperation(cardOperation: { () in
      ...
      .then{(response : Data) -> Promise<Data> in
        let keyIndex = HexHelper.makeShort(src : response.bytes, srcOff : 0)
        let keyLen = HexHelper.makeShort(src : response.bytes, srcOff : 2)
  ...
  func getKeyFromKeyChain(keyMac: String) {
    ...
    let macBytes = HexHelper.hexStrToUInt8Array(hexStr: keyMac)
    apduRunner.setCardOperation(cardOperation: { () in
      Promise<Data> { promise in
      ...
      .then{(response : Data) -> Promise<String> in
        let keyLen = HexHelper.makeShort(src : response.bytes, srcOff : 2)
        print("keyIndex = " + String(HexHelper.makeShort(src : response.bytes, srcOff : 0)))
        print("keyLen = " + String(keyLen))
  ...
  func getNumberOfKeys() {
    ...
    apduRunner.setCardOperation(cardOperation: { () in
      ...
      .then{(response : Data)  -> Promise<String> in
        return Promise<String> { promise in promise.fulfill(String(HexHelper.makeShort(src: response.bytes, srcOff: 0)))}
      }
    })
    apduRunner.startScan()
  }
  ...
  func checkKeyHmacConsistency(keyHmac: String){
    ...
    let keyHmacBytes = HexHelper.hexStrToUInt8Array(hexStr: keyHmac)
    apduRunner.setCardOperation(cardOperation: { () in
      self.selectTonAppletAndGetSault()
        .then{(sault : Data) -> Promise<Data> in
